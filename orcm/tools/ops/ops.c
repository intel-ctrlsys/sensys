...
static int gather_active_jobs(orte_ps_mpirun_info_t *hnpinfo) {
    ...
    if (ORTE_SUCCESS != (ret = orte_util_comm_query_job_info(ORTE_PROC_MY_HNP, orte_ps_globals.jobid,
...
static int gather_nodes(orte_ps_mpirun_info_t *hnpinfo) {
    ...
    if (ORTE_SUCCESS != (ret = orte_util_comm_query_node_info(ORTE_PROC_MY_HNP, NULL,
...
static int gather_vpid_info(orte_ps_mpirun_info_t *hnpinfo) {
    ...
    for(i=0; i < hnpinfo->num_jobs; i++) {
        ...
        if (ORTE_SUCCESS != (ret = orte_util_comm_query_proc_info(ORTE_PROC_MY_HNP, 
                                                                  job->jobid,
                                                                  ORTE_VPID_WILDCARD, 
                                                                  &cnt, 
                                                                  &procs))) {
...
static int parseable_print(orte_ps_mpirun_info_t *hnpinfo)
{
    ...
    printf("mpirun:num nodes:%d:num jobs:%d\n",
           hnpinfo->num_nodes, hnpinfo->num_jobs-1);
