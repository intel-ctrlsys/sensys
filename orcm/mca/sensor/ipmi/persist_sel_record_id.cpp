...
#include "persist_sel_record_id.h"
...
const int persist_sel_record_id::MAX_LINE_SIZE = 80;
...
//////////////////////////////////////////////////////////////////////////////
// Public API
persist_sel_record_id::persist_sel_record_id(const char* hostname,
...
persist_sel_record_id::~persist_sel_record_id()
{
    save_latest_record_id();
}

void persist_sel_record_id::save_latest_record_id()
{
    if("" == storage_) {
        return;
    }
...
void persist_sel_record_id::load_last_record_id(const char* name)
{
...
void persist_sel_record_id::set_record_id(uint16_t record_id)
{
...
uint16_t persist_sel_record_id::get_record_id() const
{
    return record_id_;
}
...
bool persist_sel_record_id::check_name_for_null_or_empty(const char* name)
{
    if(NULL == name) {
        return false;
    }
...
std::string persist_sel_record_id::get_stream_line(std::ifstream& strm) const
{
...
void persist_sel_record_id::set_record_id_from_str(const std::string& name_value_str)
{
...
void persist_sel_record_id::report_error(int level, const std::string& msg) const
{
...
std::string persist_sel_record_id::make_temp_filename() const
{
...
bool persist_sel_record_id::update_original_file(const char* tmp_name)
{
...
void persist_sel_record_id::str_trim(std::string& str) const
{
...
void persist_sel_record_id::str_equals_split(const std::string& in_str, std::string& out_left, std::string& out_right) const
{
...
bool persist_sel_record_id::copy_with_replace(std::ifstream& in_strm, std::ofstream& out_strm) const
{
    bool updated = false;
...
bool persist_sel_record_id::create_new(const char* filename) const
{
