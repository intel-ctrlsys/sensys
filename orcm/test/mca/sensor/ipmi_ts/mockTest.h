...
#ifndef MOCK_OPA_FACTORY_TESTS_H
#define MOCK_OPA_FACTORY_TESTS_H
...
{
public:
    virtual int init(dataContainer dc);
...
extern "C" {
    ...
    void * getMockedSymbol(const char *symbol) {
        if (!strcmp(symbol, "getOpaPluginName") && getPluginNameMock)
            return (void*)get_mockPluginName;
        else if (!strcmp(symbol, "initOpaPlugin") && initPluginMock)
    ...
    {
        if (mock_readdir) {
            const char *fake_name = "libopametrics_fake";
            if (fake_dirent) {
                free(fake_dirent);
                fake_dirent = 0 ;
            }
...
int mockPlugin::init(dataContainer dc)
{
    if (throwOnInit) {
