...
int ojob_daemon(int argc, char *argv[]);
static void shutdown_callback(int fd, short flags, void *arg);
static void pipe_closed(int fd, short flags, void *arg);
static char *get_ojobd_comm_cmd_str(int command);
void ojob_hnp_recv(int status, orte_process_name_t* sender,
                      opal_buffer_t *buffer, orte_rml_tag_t tag,
                      void* cbdata);
void ojob_daemon_recv(int status, orte_process_name_t* sender,
                      opal_buffer_t *buffer, orte_rml_tag_t tag,
                      void* cbdata);
...
opal_cmd_line_init_t orte_cmd_line_opts[] = {
    ...
    { NULL, 'h', NULL, "help", 0,
      &ojobd_globals.help, OPAL_CMD_LINE_TYPE_BOOL,
      "This help message" },
    { NULL, '\0', NULL, "debug-failure", 1,
      &ojobd_globals.fail, OPAL_CMD_LINE_TYPE_INT,
      "Have the specified orted fail after init for debugging purposes" },
    
    { NULL, '\0', NULL, "debug-failure-delay", 1,
      &ojobd_globals.fail_delay, OPAL_CMD_LINE_TYPE_INT,
    ...
    { "orte_daemonize", '\0', NULL, "daemonize", 0,
      &ojobd_globals.daemonize, OPAL_CMD_LINE_TYPE_BOOL,
      "Daemonize the orted into the background" },

    { "orte_debug_daemons", '\0', NULL, "debug-daemons", 0,
      &ojobd_globals.debug, OPAL_CMD_LINE_TYPE_BOOL,
    ...
    { NULL, '\0', NULL, "hnp", 0,
      &ojobd_globals.hnp, OPAL_CMD_LINE_TYPE_BOOL,
    ...
    { NULL, '\0', NULL, "tree-spawn", 0,
      &ojobd_globals.tree_spawn, OPAL_CMD_LINE_TYPE_BOOL,
      "Tree spawn is underway"},
    
    { NULL, '\0', NULL, "set-sid", 0,
      &ojobd_globals.set_sid, OPAL_CMD_LINE_TYPE_BOOL,
    ...
    { NULL, '\0', NULL, "report-uri", 1,
      &ojobd_globals.uri_pipe, OPAL_CMD_LINE_TYPE_INT,
      "Report this process' uri on indicated pipe"},

    { NULL, '\0', NULL, "singleton-died-pipe", 1,
      &ojobd_globals.singleton_died_pipe, OPAL_CMD_LINE_TYPE_INT,
    ...
    { NULL, '\0', "mapreduce", "mapreduce", 0,
      &ojobd_globals.mapreduce, OPAL_CMD_LINE_TYPE_BOOL,
      "Whether to report process bindings to stderr" },
...
int main(int argc, char *argv[])
{
    ...
    memset(&ojobd_globals, 0, sizeof(ojobd_globals));
    /* initialize the singleton died pipe to an illegal value so we can detect it was set */
    ojobd_globals.singleton_died_pipe = -1;
    /* init the failure orted vpid to an invalid value */
    ojobd_globals.fail = ORTE_VPID_INVALID;
     ...
     */
    if (ojobd_globals.debug) {
        gethostname(hostname, 100);
        fprintf(stderr, "Daemon was launched on %s - beginning to initialize\n", hostname);
    }
    
    /* check for help request */
    if (ojobd_globals.help) {
        char *args = NULL;
        args = opal_cmd_line_get_usage_msg(cmd_line);
    ...
    */
    ...
    if ((int)ORTE_VPID_INVALID != ojobd_globals.fail) {
        ojobd_globals.abort=false;
         ...
         */
        if (0 > ojobd_globals.fail) {
            ojobd_globals.fail = -1*ojobd_globals.fail;
            ojobd_globals.abort = true;
        }
        /* are we the specified vpid? */
        if ((int)ORTE_PROC_MY_NAME->vpid == ojobd_globals.fail) {
            /* if the user specified we delay, then setup a timer
             * and have it kill us
             */
            if (0 < ojobd_globals.fail_delay) {
                ORTE_TIMER_EVENT(ojobd_globals.fail_delay, 0, shutdown_callback, ORTE_SYS_PRI);
                
            } else {
                opal_output(0, "%s is executing clean %s", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                            ojobd_globals.abort ? "abort" : "abnormal termination");
    ...
    if (ORTE_PROC_IS_HNP) {
        ...
        /* setup the ojob ctrl or hnp daemon command receive function */
        orte_rml.recv_buffer_nb(ORTE_NAME_WILDCARD, ORCM_RML_TAG_HNP,
                            ORTE_RML_PERSISTENT, ojob_hnp_recv, NULL);
    }
    
    /* setup the primary daemon command receive function */
    orte_rml.recv_buffer_nb(ORTE_NAME_WILDCARD, ORTE_RML_TAG_DAEMON,
                            ORTE_RML_PERSISTENT, ojob_daemon_recv, NULL);
	...
	free(sysinfo);
        ...
        write(ojobd_globals.uri_pipe, tmp, strlen(tmp)+1); /* need to add 1 to get the NULL */
                      ...
                      void* cbdata)
{
    ...
    cmd_str = get_ojobd_comm_cmd_str(command);
