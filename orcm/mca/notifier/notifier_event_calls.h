...
#ifndef ORCM_NOTIFIER_EVENTS_CALLS_H
#define ORCM_NOTIFIER_EVENTS_CALLS_H
...
#define ORCM_NOTIFIER_LOG_0 0   /* Initial log format needed (no delay) */
#define ORCM_NOTIFIER_LOG_1 1   /* Intermediate log format needed (delay) */
#define ORCM_NOTIFIER_LOG_2 2   /* Final log format needed (at finalize) */

ORCM_DECLSPEC bool notifier_log_event_enabled(void);
ORCM_DECLSPEC void notifier_event_store(orcm_notifier_event_t *);
ORCM_DECLSPEC void notifier_trace_event(int, int, int32_t, time_t, time_t,
                                        const char *);


/*
 * Do not use this function directly: use ORCM_NOTIFIER_DEFINE_EVENT() instead
 */
static inline orcm_notifier_event_t *notifier_alloc_event(int ev_id,
                                                          const char *msg)
{
    orcm_notifier_event_t *ev;

    ev = OBJ_NEW(orcm_notifier_event_t);
    if (NULL == ev) {
        return NULL;
    }
...
static inline void notifier_count_and_log_event(orcm_notifier_event_t *ev,
                                                int ev_id,
                                                int cnt_thresh,
                                                int time_thresh) 
{
    ...
    if (ev->ev_already_traced) {
        if (now > ev->ev_time_trc + time_thresh) {
            ...
            ev->ev_cnt = 0;
            ev->ev_time_trc = now;
            notifier_trace_event(ORCM_NOTIFIER_LOG_1, ev_id, count, now, delay,
                                 ev->ev_msg);
        }
    } else {
        ...
        notifier_trace_event(ORCM_NOTIFIER_LOG_0, ev_id, count, now, now,
                             ev->ev_msg);
    }
}
...
#define ORCM_NOTIFIER_DEFINE_EVENT(i, m)                                     \
    ...
    {                                                                        \
        static orcm_notifier_event_t *prefix_ ## i = NULL;                   \
 ...
 *     ORCM_NOTIFIER_LOG_EVENT(0, 100, 1);
 * 100 and 1 are respectively the counter and time thresholds.
 * This actually expands to
 *     notifier_log_event_0(100, 1);
 */
#define ORCM_NOTIFIER_LOG_EVENT(i, c, t) notifier_event_fn_prefix(i) (c, t)


END_C_DECLS

#endif /* ORCM_NOTIFIER_EVENT_CALLS_H */
