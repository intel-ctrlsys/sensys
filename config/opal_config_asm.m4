...
if test "$opal_cv_asm_arch" != "WINDOWS" -a "$opal_cv_asm_builtin" != "BUILTIN_SYNC" -a "$opal_cv_asm_builtin" != "BUILTIN_OSX" ; then
    ...
    opal_cv_asm_file=""
    if $GREP "$opal_cv_asm_arch" "${top_opal_srcdir}/opal/asm/asm-data.txt" | $FGREP "$opal_cv_asm_format" >conftest.out 2>&1 ; then
        opal_cv_asm_file="`cut -f3 conftest.out`"
        if test ! "$opal_cv_asm_file" = "" ; then
            opal_cv_asm_file="atomic-${opal_cv_asm_file}.s"
            if test -f "${top_opal_srcdir}/opal/asm/generated/${opal_cv_asm_file}" ; then
    ...
    if test "$opal_cv_asm_file" = "" ; then
        if test ! "$PERL" = "" ; then
            ...
            opal_try='$PERL $top_opal_srcdir/opal/asm/generate-asm.pl $opal_cv_asm_arch "$opal_cv_asm_format" $top_opal_srcdir/opal/asm/base $top_opal_builddir/opal/asm/generated/$opal_cv_asm_file >conftest.out 2>&1'
